...
function ConnectButton({
  ...
  const iconBg =
    variant === "whatsapp" ? "bg-[#25D366] text-white" : "bg-zinc-800 text-white dark:bg-zinc-600";
  ...
  return (
    ...
    >
      <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl shadow-sm ${iconBg}`}>
        {icon}
      </span>
      <span className="min-w-0">
        <span className="block text-sm font-semibold">{label}</span>
        {sublabel ? <span className="mt-0.5 block truncate text-xs opacity-75">{sublabel}</span> : null}
      </span>
    </a>
  );
}
    ...
    <div className="relative mx-auto w-full max-w-4xl">
      ...
      <header className="relative mb-10 text-center sm:mb-12">
        ...
        <p className="relative mx-auto mt-4 max-w-xl text-sm leading-relaxed text-zinc-600 sm:text-base dark:text-zinc-400">
          If this app helped you find or list a place in Nashik, you can chip in to keep it running.
          Every contribution goes toward hosting and improvements—completely optional.
        </p>
        <ul className="relative mt-6 flex flex-wrap items-center justify-center gap-2 text-xs font-medium text-zinc-600 dark:text-zinc-400">
          ...
          <li className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200/80 bg-white/60 px-3 py-1 dark:border-zinc-700 dark:bg-zinc-900/50">
            <Coffee className="size-3.5 text-amber-600 dark:text-amber-400" aria-hidden />
            UPI friendly
          </li>
          <li className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200/80 bg-white/60 px-3 py-1 dark:border-zinc-700 dark:bg-zinc-900/50">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-zinc-200/80 bg-white/80 shadow-xl shadow-zinc-900/5 backdrop-blur-xl dark:border-zinc-700/80 dark:bg-zinc-900/60 dark:shadow-black/30">
        ...
        <div className="relative grid gap-8 p-6 sm:p-8 md:grid-cols-[1fr_auto] md:items-center md:gap-10 lg:p-10">
          <div>
            <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Scan &amp; pay via UPI</h2>
            <p className="mt-2 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
              Open PhonePe, Google Pay, or your bank app, scan the code, and send any amount you are
              comfortable with. No account or login required on RentSetGo.
            </p>
            ...
            {upiId ? (
              ...
              <p className="mt-6 rounded-xl border border-dashed border-zinc-300 bg-zinc-50/80 px-4 py-3 text-xs text-zinc-500 dark:border-zinc-600 dark:bg-zinc-950/40 dark:text-zinc-400">
                Add your UPI ID in <code className="rounded bg-zinc-200/80 px-1 dark:bg-zinc-800">.env.local</code> and
                replace the placeholder QR with your real code from your UPI app.
              </p>
            )}
          </div>

          <div className="flex flex-col items-center md:items-end">
            <div className="relative rounded-2xl border border-zinc-200/90 bg-white p-4 shadow-lg shadow-zinc-900/10 ring-1 ring-zinc-900/5 dark:border-zinc-600 dark:bg-zinc-950 dark:ring-white/5">
              <div
                className="pointer-events-none absolute -inset-px rounded-2xl bg-gradient-to-br from-rose-200/40 via-transparent to-emerald-200/40 opacity-60 dark:from-rose-500/20 dark:to-emerald-500/20"
                aria-hidden
              />
              <Image
                src={qrImage}
                ...
                width={200}
                height={200}
                className="relative size-48 rounded-xl object-contain sm:size-52"
                priority
              />
            </div>
  ...
  return (
    <>
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">Support RentSetGo</h2>
          <p className="mt-2 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
      ...
      <div className="mt-6 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
        ...
        <div className="text-center sm:text-left">
          <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Scan to support</p>
          {upiId ? (
            <p className="mt-2 font-mono text-sm text-emerald-800 dark:text-emerald-300">{upiId}</p>
          ) : null}
        </div>
      </div>
    </>
